...
public class SectFightAdapter extends PagerAdapter implements CardAdapter {
    ...
    public SectFightAdapter(Context context) {
    ...
    public void addCardItem(CardItem item,int total) {
        this.total = total;
        mViews.add(null);
        mData.add(item);
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        lastQuestionTime = EasySharePreference.getSectQuestionTime(context);
        final boolean sameDay = isSameDay(lastQuestionTime);//答题时判断是否是同一天
        int positionNum = EasySharePreference.getSectPositionNum(context);
        boolean haveAnswered = EasySharePreference.getSectHaveAnswered(context);
       ...
       //已答题目禁选
        ...
        if (item.getPosition() == positionNum && (haveAnswered||time>60*1000)) {
        ...
        holder.contentTextView.setText("("+item.getPosition()+"/"+total+")"+item.getQuestion());

        if (item.getPosition()==total) {
