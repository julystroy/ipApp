 ...
 */
public class CiYuanListAdapter extends RecyclerView.Adapter<CiYuanListAdapter.ViewHolder> {

    private Activity            context;
    private List<NewBase>       mList;
    private OnItemClickListener listener;

    public CiYuanListAdapter(Activity context) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_bangai_list, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        final NewBase expound = mList.get(position);
...
//            holder.tvFocus.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.icon_ct_focus, 0, 0, 0);
        ...
        holder.llLike.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ApiUtils.approveTarget(String.valueOf(expound.getId()), Constants.APPROVE_QMAN, new ApiQuestListener() {
                    @Override
        ...
        holder.ivShare.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
              //  ApiUtils.share(context, expound.getTitle(), expound.getCover_pic(), "番外", String.valueOf(expound.getId()));
                if (!CartoonApp.getInstance().isLogin(context)) {
                    return;
                }
                ApiUtils.share(context, expound.getTitle(), expound.getCover_pic(), "凡人次元", String.valueOf(expound.getId()));
            }
        });
    }
    ...
    public int getItemCount() {
        if (mList != null) {
            ...
            EasySharePreference.setBangaiCount(context,mList.size());
        }
